...
public class SecureShieldOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		Entity user = null;
		user = (Entity) world
				.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3((entity.getX()), (entity.getY()), (entity.getZ())), 3, 3, 3), e -> true)
				...
				for (Entity entityiterator : _entfound) {
					if (!(entityiterator == user)) {
						if (entityiterator.getXRot() <= 90) {
							entityiterator.setDeltaMovement(new Vec3((entityiterator.getLookAngle().x), (entityiterator.getLookAngle().y + 1),
									((0 - entityiterator.getLookAngle().z) * 1.2)));
						} else if (entityiterator.getXRot() <= 0) {
							entityiterator.setDeltaMovement(new Vec3(((0 - entityiterator.getLookAngle().x) * 1.2),
									(entityiterator.getLookAngle().y + 1), (entityiterator.getLookAngle().z)));
