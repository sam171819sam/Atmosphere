...
public class SaintEntity extends Monster implements IAnimatable {
	private AnimationFactory factory = GeckoLibUtil.createFactory(this);
	private boolean swinging;
	private long lastSwing;
	public String animationprocedure = "empty";

	public SaintEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AtmosphereModEntities.SAINT.get(), world);
	}

	public SaintEntity(EntityType<SaintEntity> type, Level world) {
		super(type, world);
		xpReward = 5;
		setNoAi(false);
	}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(SaintEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
