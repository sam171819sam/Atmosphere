...
public class SecureShieldEntity extends PathfinderMob implements IAnimatable {
	...
	public SecureShieldEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(AtmosphereModEntities.SECURE_SHIELD.get(), world);
	}

	public SecureShieldEntity(EntityType<SecureShieldEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(true);
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(SecureShieldEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
