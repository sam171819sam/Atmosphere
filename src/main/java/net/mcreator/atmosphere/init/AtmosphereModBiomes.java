...
import net.mcreator.atmosphere.world.biome.StardustVallyBiome;
...
public class AtmosphereModBiomes {
	...
	public static final RegistryObject<Biome> STARDUST_VALLY = REGISTRY.register("stardust_vally", StardustVallyBiome::createBiome);
		...
		for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
			...
			if (dimensionType == dimensionTypeRegistry.getOrThrow(BuiltinDimensionTypes.OVERWORLD)) {
				...
				if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
					...
					parameters.add(new Pair<>(StardustVallyBiome.PARAMETER_POINT,
							biomeRegistry.getOrCreateHolderOrThrow(ResourceKey.create(Registry.BIOME_REGISTRY, STARDUST_VALLY.getId()))));
				...
				if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
					...
					if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
						...
						surfaceRules.add(1,
								preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, STARDUST_VALLY.getId()),
										AtmosphereModBlocks.STARDUST_SAND.get().defaultBlockState(),
										AtmosphereModBlocks.SPARKLING_STARSTONE.get().defaultBlockState(),
										AtmosphereModBlocks.SPARKLING_STARSTONE.get().defaultBlockState()));
